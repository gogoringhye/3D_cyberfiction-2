function locomotive() {
  ...
  //locoScroll객체를 생성, 스크롤 관련 동작을 제어하는데 사용됨
  // el: document.querySelector("#main")--> #main이라는 CSS선택자로 해당하는 HTML요소를 스크롤 컨테이너로 지정(#main영역이 LocomotiveScroll이 적용되는 범위)
  //smooth: true  --> 부드러운 스크롤을 활성화

  locoScroll.on("scroll", ScrollTrigger.update); //LocomotiveScroll의 Scroll 이벤트가 발생할때마다 ScrollTrigger의 update함수를 호출, 이것은 스크롤 이벤트와 ScrollTrigger간의 연동을 설정
  ...
  ScrollTrigger.scrollerProxy("#main", {
    ...
    pinType: document.querySelector("#main").style.transform ?
      "transform" : "fixed",
  }); //pinType은 #main 요소의 스타일 속성 transform이 설정되어 있으면 transform으로 , 그렇지 않으면 "fixed"로 설정

  // "refresh" 이벤트를 감지하면 locoScroll.update() 함수를 호출하여 LocomotiveScroll을 업데이트합니다. 스크롤 컨테이너나 내용이 동적으로 변경될 때 사용됨
